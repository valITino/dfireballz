"""Threat Intelligence MCP Server.

VirusTotal, Shodan, AbuseIPDB, MalwareBazaar, ThreatFox, URLScan.
"""

import os

import requests
from fastmcp import FastMCP

mcp = FastMCP(
    "threat-intel",
    instructions="Threat intel: VirusTotal, Shodan, AbuseIPDB, MalwareBazaar, ThreatFox, URLScan",
)


def _get_key(service: str) -> str:
    """Get API key from environment."""
    env_var = f"{service.upper()}_API_KEY"
    key = os.environ.get(env_var, "").strip()
    if not key:
        raise ValueError(
            f"{service} API key not configured. "
            f"Set {env_var} in .env (not empty), then run: "
            f"make restart-threat"
        )
    return key


# ── Raw implementation functions (callable from enrich_ioc) ──────────


def _vt_lookup(indicator: str, ioc_type: str = "file_hash") -> dict:
    """Look up an indicator on VirusTotal."""
    api_key = _get_key("virustotal")
    base = "https://www.virustotal.com/api/v3"
    headers = {"x-apikey": api_key}

    endpoints = {
        "file_hash": f"{base}/files/{indicator}",
        "url": f"{base}/urls/{indicator}",
        "domain": f"{base}/domains/{indicator}",
        "ip": f"{base}/ip_addresses/{indicator}",
    }

    if ioc_type not in endpoints:
        return {"error": f"Invalid type. Must be one of: {list(endpoints.keys())}"}

    try:
        resp = requests.get(endpoints[ioc_type], headers=headers, timeout=30)
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _shodan_host(ip: str) -> dict:
    """Get Shodan host information for an IP address."""
    api_key = _get_key("shodan")
    try:
        resp = requests.get(
            f"https://api.shodan.io/shodan/host/{ip}",
            params={"key": api_key},
            timeout=30,
        )
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _shodan_search(query: str, page: int = 1) -> dict:
    """Search Shodan with a query string."""
    api_key = _get_key("shodan")
    try:
        resp = requests.get(
            "https://api.shodan.io/shodan/host/search",
            params={"key": api_key, "query": query, "page": page},
            timeout=30,
        )
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _abuse_ip_check(ip: str) -> dict:
    """Check IP reputation on AbuseIPDB."""
    api_key = _get_key("abuseipdb")
    try:
        resp = requests.get(
            "https://api.abuseipdb.com/api/v2/check",
            headers={"Key": api_key, "Accept": "application/json"},
            params={"ipAddress": ip, "maxAgeInDays": 90, "verbose": True},
            timeout=30,
        )
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _malware_bazaar_lookup(hash_or_tag: str) -> dict:
    """Search MalwareBazaar for a sample by hash or tag."""
    try:
        # Try hash lookup first
        resp = requests.post(
            "https://mb-api.abuse.ch/api/v1/",
            data={"query": "get_info", "hash": hash_or_tag},
            timeout=30,
        )
        result = resp.json()
        if result.get("query_status") == "hash_not_found":
            # Try tag search
            resp = requests.post(
                "https://mb-api.abuse.ch/api/v1/",
                data={"query": "get_taginfo", "tag": hash_or_tag, "limit": 25},
                timeout=30,
            )
            result = resp.json()
        return result
    except requests.RequestException as e:
        return {"error": str(e)}


def _threatfox_lookup(ioc: str) -> dict:
    """Search ThreatFox IOC database."""
    try:
        resp = requests.post(
            "https://threatfox-api.abuse.ch/api/v1/",
            json={"query": "search_ioc", "search_term": ioc},
            timeout=30,
        )
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _urlscan_lookup(url: str) -> dict:
    """Submit a URL to URLScan.io for analysis."""
    api_key = _get_key("urlscan")
    try:
        resp = requests.post(
            "https://urlscan.io/api/v1/scan/",
            headers={"API-Key": api_key, "Content-Type": "application/json"},
            json={"url": url, "visibility": "private"},
            timeout=30,
        )
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _cve_lookup(cve_id: str) -> dict:
    """Look up CVE details from NVD."""
    try:
        resp = requests.get(
            "https://services.nvd.nist.gov/rest/json/cves/2.0",
            params={"cveId": cve_id},
            timeout=30,
        )
        resp.raise_for_status()
        return resp.json()
    except requests.RequestException as e:
        return {"error": str(e)}


def _enrich_ioc(ioc: str, ioc_type: str = "auto") -> dict:
    """Enrich an IOC by querying all relevant threat intelligence sources."""
    results: dict = {"ioc": ioc, "type": ioc_type, "sources": {}}

    # Auto-detect type
    if ioc_type == "auto":
        if ioc.count(".") == 3 and all(p.isdigit() for p in ioc.split(".")):
            ioc_type = "ip"
        elif ioc.startswith("http"):
            ioc_type = "url"
        elif len(ioc) in (32, 40, 64) and all(
            c in "0123456789abcdefABCDEF" for c in ioc
        ):
            ioc_type = "file_hash"
        elif "@" in ioc:
            ioc_type = "email"
        else:
            ioc_type = "domain"
        results["type"] = ioc_type

    def _try_source(name: str, func, *args) -> None:
        try:
            results["sources"][name] = func(*args)
        except Exception as e:
            results["sources"][name] = {"error": str(e)}

    if ioc_type == "ip":
        _try_source("shodan", _shodan_host, ioc)
        _try_source("abuseipdb", _abuse_ip_check, ioc)
        _try_source("virustotal", _vt_lookup, ioc, "ip")
        _try_source("threatfox", _threatfox_lookup, ioc)

    elif ioc_type == "domain":
        _try_source("virustotal", _vt_lookup, ioc, "domain")
        _try_source("threatfox", _threatfox_lookup, ioc)

    elif ioc_type == "file_hash":
        _try_source("virustotal", _vt_lookup, ioc, "file_hash")
        _try_source("malware_bazaar", _malware_bazaar_lookup, ioc)
        _try_source("threatfox", _threatfox_lookup, ioc)

    elif ioc_type == "url":
        _try_source("virustotal", _vt_lookup, ioc, "url")
        _try_source("urlscan", _urlscan_lookup, ioc)

    # Compute confidence score based on number of positive results
    positive_sources = sum(
        1
        for v in results["sources"].values()
        if isinstance(v, dict) and "error" not in v
    )
    total_sources = len(results["sources"])
    results["confidence"] = int((positive_sources / max(total_sources, 1)) * 100)

    return results


# ── MCP tool registrations (thin wrappers around raw functions) ──────


@mcp.tool()
def vt_lookup(indicator: str, ioc_type: str = "file_hash") -> dict:
    """Look up an indicator on VirusTotal.

    Args:
        indicator: The indicator value (hash, URL, domain, or IP)
        ioc_type: Indicator type — file_hash | url | domain | ip
    """
    return _vt_lookup(indicator, ioc_type)


@mcp.tool()
def shodan_host(ip: str) -> dict:
    """Get Shodan host information for an IP address.

    Args:
        ip: IP address to look up
    """
    return _shodan_host(ip)


@mcp.tool()
def shodan_search(query: str, page: int = 1) -> dict:
    """Search Shodan with a query string.

    Args:
        query: Shodan search query (dork)
        page: Result page number
    """
    return _shodan_search(query, page)


@mcp.tool()
def abuse_ip_check(ip: str) -> dict:
    """Check IP reputation on AbuseIPDB.

    Args:
        ip: IP address to check
    """
    return _abuse_ip_check(ip)


@mcp.tool()
def malware_bazaar_lookup(hash_or_tag: str) -> dict:
    """Search MalwareBazaar for a sample by hash or tag.

    Args:
        hash_or_tag: SHA256/MD5/SHA1 hash or malware tag
    """
    return _malware_bazaar_lookup(hash_or_tag)


@mcp.tool()
def threatfox_lookup(ioc: str) -> dict:
    """Search ThreatFox IOC database.

    Args:
        ioc: IOC to search (IP, domain, URL, hash)
    """
    return _threatfox_lookup(ioc)


@mcp.tool()
def urlscan_lookup(url: str) -> dict:
    """Submit a URL to URLScan.io for analysis.

    Args:
        url: URL to analyze
    """
    return _urlscan_lookup(url)


@mcp.tool()
def cve_lookup(cve_id: str) -> dict:
    """Look up CVE details from NVD.

    Args:
        cve_id: CVE identifier (e.g., CVE-2024-1234)
    """
    return _cve_lookup(cve_id)


@mcp.tool()
def enrich_ioc(ioc: str, ioc_type: str = "auto") -> dict:
    """Enrich an IOC by querying all relevant threat intelligence sources.

    Args:
        ioc: The indicator of compromise value
        ioc_type: IOC type — auto | ip | domain | url | file_hash | email
    """
    return _enrich_ioc(ioc, ioc_type)


if __name__ == "__main__":
    mcp.run(transport="stdio")
